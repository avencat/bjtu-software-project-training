...
function createUser(req, res, next) {
  ...
  if (!body.password || body.password.length < 6) {
    ...
    db.none(format('INSERT into users(firstname, lastname, birthday, email, login, telephone, password, created, updated, following_nb, follower_nb)' +
      'values(%1$L, %2$L, %3$L, %4$L, %5$L, %6$L, %7$L, %8$L, %8$L, 0, 0)',
      body.firstname, body.lastname, body.birthday, body.email, body.login, body.telephone, body.password, body.created))
      .then(() => {
        res.status(201)
          .json({
...
function getSingleUser(req, res, next) {
  ...
  db.oneOrNone(format(request + ' FROM users ' +
